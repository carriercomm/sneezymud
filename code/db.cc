...
//  buildSpellDamArray();
  ...
  for (i = 0;i < obj_index.size();i++) {
    if ((obj = read_object(i, REAL)) != NULL) {
      // loading it inits everything
      obj->boottimeInit();

      delete obj;
      obj = NULL;
    }
    // more boot time checks
    // check strings for items being abbreviations of other items
    // this causes problems in shop functionality - bat 5/22/97

#if 0

    if (gamePort != PROD_GAMEPORT) {
      for (j = 0; j < obj_index.size(); j++) {
        if (i == j)
          continue;
        if (isname(obj_index[i].name, obj_index[j].name)) {
          vlogf(LOG_LOW, "Obj (%s:%d) is abbreviation of obj (%s:%d)",
           obj_index[i].name, obj_index[i].virt,
           obj_index[j].name, obj_index[j].virt);
        }
      }
      for (j = 0; j < mob_index.size(); j++) {
        if (isname(obj_index[i].name, mob_index[j].name)) {
          vlogf(LOG_LOW, "Obj (%s:%d) is abbreviation of mob (%s:%d)",
             obj_index[i].name, obj_index[i].virt,
             mob_index[j].name, mob_index[j].virt);
        }
      }
    }
#endif
