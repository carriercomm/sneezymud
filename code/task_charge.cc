...
int taskChargeMoveInto(int to_room, TBeing *ch, bool moveHorse)
{
  ...
  if (!moveHorse) {
    ...
    if (ch->bothLegsHurt()) {
      ch->sendTo(COLOR_MOBS, "Riding %s without working legs is painful!\n\r",
                 tHorse->getName());
      ch->addToMove(-5);
      if (!::number(0, 1)) {
...
int ChargeFlyIntoRoom(TBeing *ch, roomDirData *rExit)
{
  ...
  ch->sendTo("You fly into the next room and smash into the %s.\n\r",
             real_roomp(nRoom)->describeGround().c_str());
  ch->doLook("", CMD_LOOK);
...
int ChargeHitDoor(TBeing *ch, roomDirData *rExit)
{
  ...
  tHorse = dynamic_cast<TBeing *>(ch->riding);
  ch->sendTo("You charge towards %s!\n\r",
