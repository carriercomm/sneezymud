...
void TBeing::doConsider(const sstring &argument)
{
  ...
  if (victim == this) {
      ...
      diff = suggest - armor;
      sendTo(fmt("Your equipment would seem %s for your class and level.\n\r") %
  ...
  if (victim->isPc() && victim->isImmortal()) {
    ...
    if (isImmortal() && hasWizPower(POWER_IMM_EVAL)) {
      ...
      sendTo(fmt("You consider %s's equipment...\n\r") % victim->getName());
      sendTo(fmt("%s should have an AC of [%d] but has an AC of [%d]\n\r") %
  ...
  if (getDiscipline(DISC_ADVENTURING)) {
    ...
    if (tmon->isAnimal() && doesKnowSkill(SKILL_CONS_ANIMAL)) {
      ...
      sendTo(COLOR_MOBS, fmt("Using your knowledge of animal lore, you determine that %s is %s %s.\n\r") %
            namebuf %
