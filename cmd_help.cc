...
void TBeing::displayHelpFile(char *helppath, char *namebuf){
  ...
  str = fmt("%s%-30.30s (Last Updated: %s)%s\n\r\n\r") %
    green() % namebuf % timebuf % norm();
...
void TBeing::doHelp(const char *arg)
{
  ...
  if (found) {
    ...
    if (disc_num != DISC_NONE) {
      ...
      str += purple();
      if (isImmortal()) {
        str += fmt("    (disc: %d, spell %d)") % mapDiscToFile(disc_num) % skill;
      }
    } else {
    ...
    str += purple();
    if (isImmortal()) {
      str += fmt("    (disc: %d)") % mapDiscToFile(discArray[skill]->assDisc);
    }
    ...
    str += fmt("%d%%") % discArray[skill]->start;
    str += purple();
    ...
    if (isImmortal()) {
      str += fmt(" (%d)") % discArray[skill]->learn;
    }
    str += "\n\rLearn By Doing   : ";
    str += norm();
    ...
    if (isImmortal()) {
      str += fmt("  %s(%d) (%d) %s") % purple() % discArray[skill]->startLearnDo % discArray[skill]->amtLearnDo % norm();
    }
  ...
  // this is calculated differently now, so lets not mislead folks
    ...
    if (discArray[skill]->minLifeforce) {
      if (doesKnowSkill(skill) && (IS_SET(discArray[skill]->comp_types, SPELL_TASKED))) {
        ...
        str += fmt("%sCurrent Lifeforce:%s %d, per round amount : %d\n\r") %
          purple() % norm() %
          (useLifeforce(skill) * (discArray[skill]->lag + 2)) %
          useLifeforce(skill);
      } else if (doesKnowSkill(skill)) {
        str += fmt("%sMinimum Lifeforce:%s %d, current : %d\n\r") %
          purple() % norm() % discArray[skill]->minLifeforce %
          useLifeforce(skill);
      } else {
        str += fmt("%sLifeforce:min/cur:%s %d/spell-not-known\n\r") %
          purple() % norm() % discArray[skill]->minLifeforce;
      }
    }
    ...
    str += fmt("%sOffensive        : %s%s\t%sArea Effect          : %s%s\n\r") %
      purple() % norm() %
      ((discArray[skill]->targets & TAR_VIOLENT) ? "Yes" : "No") %
      purple() % norm() %
      ((discArray[skill]->targets & TAR_AREA) ? "Yes" : "No");

    str += fmt("%sCast on Self     : %s%s\t%sObject Castable      : %s%s\n\r") %
      purple() % norm() %
  ...
  if (found) {
    ...
    if (disc_num != DISC_NONE) {
      ...
      str += purple();
      if (isImmortal()) {
        str += fmt("    (disc: %d, skill %d)") % mapDiscToFile(disc_num) % skill;
      }
    } else {
    ...
    str += learn_name(discArray[skill]->learn);
    if (isImmortal()) {
      str += fmt("%s(%d)%s") % purple() % discArray[skill]->learn % norm();
    }
    ...
    if (isImmortal()) {
      str += fmt("  %s(%d) (%d) %s") %
        purple() % discArray[skill]->startLearnDo %
        discArray[skill]->amtLearnDo % norm();
    }
    str += "\n\r";
    ...
    lag_t lag = discArray[skill]->lag;
    if (lag > LAG_0) {
      str += fmt("%sCommand lock-out :%s %.1f seconds") %
