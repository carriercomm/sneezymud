...
// set the players time
...
#endif
     sendTo(COLOR_MOBS, "You have set %s to level %d.\n\r", tper->getName(), parm);
     sendTo("DoneBasic is set at %d for that class.\n\r", tper->player.doneBasic[p2ci]);
     tper->sendTo(COLOR_MOBS, "You have been set %s to level %d.\n\r", tper->getName(), parm);
     tper->doSave(SILENT_YES);
#endif
  } else if (is_abbrev(field, "class")) {
    sscanf(parmstr, "%d", &parm);
    if (parm <= 0 || parm >= (1<<MAX_CLASSES)) {
      sendTo("Class must be in the range 1-%d.\n\r", (1<<MAX_CLASSES)-1);
      return;
    }
    mob->setClass(parm);
    if (mob->isImmortal()) {
    ...
    sendTo(COLOR_MOBS, "%s's title is now: %s\n\r", tper->getName(), tper->title);
    return;
    ...
    if (fnum != FACT_UNDEFINED) {
      mob->setFaction(fnum);
      sendTo(COLOR_MOBS, "You just changed %s's faction to %s.\n\r",mob->getName(),
    ...
    if (parm < 0 || parm >= FACT_LEADER_SLOTS) {
      sendTo("Leader slot in range 0 to %d.\n\r", FACT_LEADER_SLOTS - 1);
      sendTo("Syntax: @set leader <x> <faction> <leader> <leader slot>\n\r");
      sendTo("Syntax: @set leader <x> <faction> Noone <leader slot>\n\r");
      return;
    }
    ...
    sendTo(COLOR_MOBS, "%s grew up as a %s.\n\r", mob->getName(),
                home_terrains[parm]);
#if FACTIONS_IN_USE
    ...
    if (!mob->getDiscipline(dnt)) {
      sendTo(COLOR_MOBS, "Sorry, but %s doesn't have that discipline valid.\n\r", mob->getName());
      return;
    }
    ...
    sendTo(COLOR_MOBS, "%s now has a current limb health of %d on %s %s.\n\r",
          mob->getName(), mob->getCurLimbHealth(wearSlotT(parm)), mob->hshr(), mob->describeBodySlot(wearSlotT(parm)).c_str());
    ...
    if (!strcmp(parmstr, "maximum")) {
      ...
      mob->affectTotal();
      sendTo(COLOR_MOBS, "Setting %s's skill set to %d percent of his max in each skill.\n\r",mob->getName(), parm2);
      return;
    }
#endif
...
// These kludges may not be necessary now, just use a .
    ...
    if (!(mob->getSkill(snt))) {
       if (discArray[snt])
         ...
         sendTo(COLOR_MOBS, "%s doesnt appear to have that skill.\n\r",
mob->getName());
       return;
    }
    ...
    sendTo(COLOR_MOBS, "Setting %s's skill: %s to %d.\n\r",mob->getName(),
    ...
    if (parm2 < 0 || parm2 >= MAX_CLASSES) {
      sendTo("Syntax: @set level <char name> <level> <class number>\n\r");
      sendTo("<class number> must be between 0 and %d.\n\r", MAX_CLASSES-1);
      return;
    }

    if (dynamic_cast<TPerson *>(mob)) {
      if ((mob->GetMaxLevel() >= GetMaxLevel()) && (this != mob)) {
        mob->sendTo(COLOR_MOBS, "%s just tried to change your level.\n\r", getName());
        return;
      }
    } else {
      if (parm2 < MIN_CLASS_IND || parm2 >= MAX_CLASSES) {
    ...
    if (parm < GetMaxLevel() ||
      ...
      if (hasWizPower(POWER_SET_IMP_POWER)) {
        ...
        if (parm > MAX_MORT) {
          sendTo("Sorry, you can't advance past %dth level.\n\r", MAX_MORT);
          return;
        }
        if (parm2 >= MIN_CLASS_IND && parm2 < MAX_CLASSES) {
          mob->setLevel(classIndT(parm2), parm);
    ...
    sendTo(COLOR_MOBS, "You set %s's strength (chosen: %d) (cur: %d).\n\r", 
            mob->getName(), 
    ...
    sendTo(COLOR_MOBS, "You set %s's brawn (chosen: %d) (cur: %d).\n\r", 
            mob->getName(), 
    ...
    sendTo(COLOR_MOBS, "You set %s's dexterity (chosen: %d) (cur: %d).\n\r", 
            mob->getName(), 
    ...
    sendTo(COLOR_MOBS, "You set %s's agility (chosen: %d) (cur: %d).\n\r", 
            mob->getName(), 
    ...
    sendTo(COLOR_MOBS, "You set %s's constitution (chosen: %d) (cur: %d).\n\r", 
            mob->getName(), 
    ...
    sendTo(COLOR_MOBS, "You set %s's intellect (chosen: %d) (cur: %d).\n\r", 
            mob->getName(), 
    ...
    sendTo(COLOR_MOBS, "You set %s's wisdom (chosen: %d) (cur: %d).\n\r", 
            mob->getName(), 
    ...
    sendTo(COLOR_MOBS, "You set %s's focus (chosen: %d) (cur: %d).\n\r", 
            mob->getName(), 
    ...
    sendTo(COLOR_MOBS, "You set %s's karma (chosen: %d) (cur: %d).\n\r", 
            mob->getName(), 
    ...
    sendTo(COLOR_MOBS, "You set %s's charisma (chosen: %d) (cur: %d).\n\r", 
            mob->getName(), 
    ...
    sendTo(COLOR_MOBS, "You set %s's speed (chosen: %d) (cur: %d).\n\r", 
            mob->getName(), 
    ...
    sendTo(COLOR_MOBS, "You set %s's perception (chosen: %d) (cur: %d).\n\r", 
            mob->getName(), 
    ...
    sendTo(COLOR_MOBS, "You set %s's piety to %.3f.\n\r", mob->getName(), mob->getPiety());
  } else if (is_abbrev(field, "mmana")) {
    sscanf(parmstr, "%d", &parm);
    mob->setMaxMana(parm);
    sendTo(COLOR_MOBS, "You set %s's max mana to %d.\n\r", mob->getName(), mob->manaLimit());
    ...
    sscanf(parmstr, "%d", &parm);
    mob->setMaxMove(parm);
    sendTo(COLOR_MOBS, "You set %s's max move to %d.\n\r", mob->getName(), mob->moveLimit());
  } else if (is_abbrev(field, "move")) {
    sscanf(parmstr, "%d", &parm);
    mob->setMove(parm);
    sendTo(COLOR_MOBS, "You set %s's move to %d.\n\r", mob->getName(), mob->getMove());
    ...
    if (parm >= MAX_WEAR ||
        ...
        parm == HOLD_LEFT) {
      sendTo("Syntax : @set stuck <playername> <object> <body part>\n\r");
      sendTo("Body part must be in range %d to %d.\n\r", MIN_WEAR, MAX_WEAR - 1);
      return;
    }
    ...
    if (parm < MIN_WEAR || parm >= MAX_WEAR) {
      sendTo("Syntax: @set bodyflags <char name> <part #> <flags>.\n\r");
      sendTo("Please use numbers between %d and %d for <part #>.\n\r", MIN_WEAR, MAX_WEAR - 1);
      return;
    }
