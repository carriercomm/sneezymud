...
void task_whittleSetupObject(TBeing *ch, TObj *tObj, TOrganic *tWood, int tIndex)
{
  ...
  if ((tArrow = dynamic_cast<TArrow *>(tObj)) ||
    ...
    two_arg(tStString, tStPost, tStWood);
...
bool task_whittleCreateNew(TBeing *ch, string tStWood, int tIndex)
{
  TOrganic *tWood        = NULL,
           *tOldWood     = NULL;
  TThing   *tObjTemp     = (ch ? ch->stuff : NULL),
           *tObjTempNext = (ch ? ch->stuff : NULL);
...
void TBeing::doWhittle(const char *tArg)
{
  ...
  */
  ...
  if (!tArg || !*tArg) {
    ...
    if (isImmortal()) {
      ...
      for (int whittleIndex = 0; whittleIndex < (signed) whittleItems.size(); whittleIndex++) {
        ...
        sendTo("%-45s:%c: %6.0f %3.0f %3.0d%% %5d %2d %4.0f\n\r",
               whittleItems[whittleIndex].name.c_str(),
  ...
  tStString = two_arg(tStString, tStObject, tStWood);
  ...
  if (tStWood.empty()) {
    if (!(tObj = searchLinkedListVis(this, tStObject.c_str(), stuff))) {
...
string taskWhittleEntry::getName(bool showSecond)
{
  ...
  if (showSecond) {
    tStString = two_arg(tStString, tStName, tStExcess);
    tStString = two_arg(tStString, tStName, tStExcess);
    strcpy(tName, tStName.c_str());
  } else {
    two_arg(tStString, tStName, tStExcess);
    strcpy(tName, tStName.c_str());
  }
