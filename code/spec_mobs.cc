...
{
  return (searchLinkedList((char *) c_data, real_roomp(room)->stuff, TYPEOBJ) != NULL);
}

// could be used by find_path
int named_mob_on_ground(int room, void *c_data)
{
  return (searchLinkedList((char *) c_data, real_roomp(room)->stuff, TYPEMOB) != NULL);
}

// could be used by find_path
int named_being_on_ground(int room, void *c_data)
{
  return (searchLinkedList((char *) c_data, real_roomp(room)->stuff, TYPEBEING) != NULL);
}
  ...
  switch (request) {
    ...
    case 2:
      ...
      if (ch->affectedBySpell(AFFECT_NEWBIE)) {
        ...
        best = NULL;
        for (i = ch->stuff; i && !best; i = i->nextThing) {
          i->findSym(&best);

          for (j = i->stuff; j && !best; j = j->nextThing) {
            j->findSym(&best); 
          }
        }
        if (best) {
    ...
    case 3:
      ...
      for (i = ch->stuff; i; i = i->nextThing) {
    ...
    if ((targ = ch->findAnAttacker()) && !targ->isImmortal()) {
      ...
      if (targ->getHeight() <= (ch->getHeight()*3)) {
        if (!::number(0, 20)) {
          ...
          TBaseCorpse *co = NULL;
          if ((co = dynamic_cast<TBaseCorpse *>(ch->roomp->stuff))) {
            TThing *t;
            for (t = co->stuff; t; t = co->stuff) {
              --(*t);
              *ch += *t; 
            }
  ...
  if (ch->fight() && ch->fight()->isPc() && ch->canSpeak()) {
    ...
    if (ch->canSee(ch->fight())) {
      ...
      switch (number(1, 145)) {
        ...
        case 12:
        case 13:
          strcpy(buf, "HELP!  I am being attacked.");
          break;
        case 14:
        case 15:
        case 16:
          strcpy(buf, "BANZAI!  CHARGE!  DEATH BEFORE DISHONOR!  DIE DIE DIE!  SPORK!");
          break;
        case 17:
        case 18:
          strcpy(buf, "Aaaeeeiiih!!");
          break;
        case 19:
        ...
        case 40:
        case 41:
          sprintf(buf, "I need some backup here, come to %s!", ch->roomp->getName());
          break;
        case 42:
        case 43:
        case 44:
        ...
        case 63:
          ...
          break;
        case 64:
          strcpy(buf, "Have you ever danced with the devil under the pale moonlight?");
          break;
        case 65:
        case 66:
        ...
        case 87:
        case 88:
          strcpy(buf, "Yaaaaarrrggggggghhhhhh!");
          break;
        case 89:
        case 90:
        case 91:
        ...
        case 108:
        case 109:
          sprintf(buf, "Ruffians at %s!", ch->roomp->name);
          break;
        case 110:
        case 111:
        case 112:
...
int attuner(TBeing *ch, cmdTypeT cmd, const char *arg, TMonster *me, TObj *o)
{
  ...
  switch (cmd) {
    ...
    case CMD_GENERIC_PULSE:
      for (t = me->stuff; t; t = t->nextThing) {
        if (t != job->sym)
          continue;
        else {
          found = TRUE;
...
int death(TBeing *, cmdTypeT cmd, const char *, TMonster *me, TObj *)
{
  ...
  TThing *t1, *t2;
  for (t1 = me->roomp->stuff;t1; t1 = t2) {
    t2 = t1->nextThing;
...
int grimhavenHooker(TBeing *ch, cmdTypeT cmd, const char *, TMonster *myself, TObj *)
{
  ...
  if (!myself->act_ptr) {
    ...
    // find a john
    for(t=myself->roomp->stuff; t && !found; t=t->nextThing){
