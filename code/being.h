...
const unsigned long PLR_UNUSED4    = (1<<24);
...
const unsigned long AFF_UNDEF4            = (1<<24);       // 16777216
...
class TBeing : public TThing {
  ...
  public:
    ...
    const string default_body_slot(wearSlotT) const;
    const string defaultEquipmentSlot(wearSlotT) const;
    const string slotPlurality(int) const;
    const string describeBodySlot(wearSlotT) const;
    const string describeBodySlot2(wearSlotT) const;
    const string describeEquipmentSlot(wearSlotT) const;
    int shouldDescTransLimb(wearSlotT) const;
    bool isTransformableLimb(wearSlotT, int);
    const string describeTransLimb(wearSlotT) const;
    const string describeTransBodySlot(wearSlotT) const;
    const string describeTransEquipSlot(wearSlotT) const;
    void describeImmunities(TBeing *, int);
    ...
    void genericKillFix();
    virtual int genericMovedIntoRoom(TRoom *, sh_int);
    ...
    int parseCommand(const char *, bool);
    ...
    void addObjCost(TBeing *, TObj *, objCost *, string &str); 
    ...
    string dynColorRoom(TRoom *, int, bool) const;
    ...
    void doSit(const string &);
    void doRest(const string &);
    void doSleep(const string &);
    ...
    int doGoto(const string &);
    int doMortalGoto(const string &);
